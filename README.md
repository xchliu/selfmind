...
**个人专属 AI 记忆库 — 可见 · 可感知 · 可干预**

基于认知心理学的 AI 记忆系统。双引擎驱动：向后看——记忆图谱让一切可见可感知；向前看——认知差距检测主动探测"你应该知道什么"，标记盲区，缩小信息差。
...
SelfMind 不是单一的记忆工具，而是两个引擎的叠加：

**引擎1：记忆可视化（向后看）**
记录和展示你已经沉淀的知识——图谱、DNA、健康指标、演变轨迹。回答"我记得什么"。

**引擎2：认知差距检测（向前看）**
先界定"你应该知道什么"，再对比已有图谱，标记盲区。自动感知认知缺口，每天≤3条提醒。回答"我还缺什么"。

两个引擎共用同一套数据管道（Memory/Honcho/Wiki/Skills），
但输出完全不同：一个展示存量，一个驱动增量。

| 功能 | 说明 |
|------|------|
| 🧠 记忆图谱 | D3力导向图可视化，节点分类聚类，时间线播放记忆演变 |
| 🧬 Agent切换 | 标题区下拉菜单一键切换不同Agent，图谱+DNA+健康数据联动刷新 |
...
| 🔍 认知差距 | 主动界定"你应该知道什么"，从OKR/项目/Memory推理信号B，匹配图谱输出红/黄/绿标，每日≤3条盲区提醒 |
