...
| Wiki库 | 结构化知识卡片+详情弹窗+编辑保存 | ✅ 已完成（原知识图谱tab改造） |
| 记忆健康 | 遗忘曲线、衰减预警 | ✅ 已修复激活（decay公式修正+启动自动sync） |
| 时间轴 | 记忆时间线播放 | ✅ 基础完成 |
| **焦点模式** | 时间线播放自动对焦变化区域 | ✅ 已完成 |
| 主题切换 | 浅色/深色主题 | ✅ 浅色版完成 |
| **实时感知** | 源文件变化自动检测与图谱刷新 | ✅ 已完成 |
| **记忆沉淀** | U型6层沉淀路径+激活路径可视化 | ✅ 已完成 |
| **演变追踪** | 记忆产生时间+版本+更新时间+记忆强度 | 🔄 设计方向确定 |
| **图谱逐级展开** | 节点按层级逐步展开，减少初始渲染负担 | ✅ 已完成 |
...
- [ ] 修复边关系逻辑（当前0条边）
- [ ] 演变追踪实现（核心字段：产生时间+版本+更新时间+记忆强度，数据源无产生时间则用采集时间）
- [ ] 完善记忆健康可视化
- [ ] 浅色主题重写
- [ ] 节点交互优化（拖拽、缩放）
...
| 批量操作 | 批量编辑、删除、归类 | ❌ 未实现 |
| 高级搜索 | 语义搜索、多条件过滤 | 🔄 简单 |
| **数据管道统一** | unified_store + unified_sync，SQLite统一数据源 | 🔄 进行中（bulk_upsert有bug，Honcho数据未sync，前端健康页待适配） |

### 技术任务

- [x] 数据管道统一起步（unified_store.py + unified_sync.py，SQLite为统一数据源）
- [ ] 记忆 CRUD API
- [ ] 分类/标签管理界面
- [ ] 导入导出功能（JSON/MD）
- [ ] 批量选择与操作
- [ ] 富文本编辑器
- [ ] 记忆关联推荐
- [ ] unified_store bulk_upsert bug修复
- [ ] Honcho数据sync接入unified_sync
- [ ] 前端健康页适配统一数据管道
...
- 冲突解决机制（自动 + 人工）
- 可嵌入其他 Agent 系统
- 每条记忆附带演变追踪信息（产生时间、版本、更新时间、记忆强度）
...
| 版本 | 时间 | 核心交付 |
|------|------|----------|
| v1.0 | 当前 ✅ | Wiki库 + 记忆健康激活 + 文件整理 + Docker化 |
| v1.1 | +2周 | 数据管道统一稳定 + 边关系修复 + 自动sync机制 |
| v1.2 | +1月 | 演变追踪实现 + 交互体验完善 + 图谱逐级展开优化 |
...
├── data/
│   ├── data.json
│   └── selfmind.db     (SQLite统一数据源)
├── docs/               (11个md文件)
├── selfmind_app/
│   ├── handlers/        (4个mixin，从1782行http_handler拆分而来)
│   ├── providers/
│   ├── unified_store.py (统一存储层)
