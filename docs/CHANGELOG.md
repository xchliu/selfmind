...
### Added
- 🧠 **认知差距检测（Should-Know）** — 从"我知道什么"升级到"我应该知道什么"，主动界定认知边界
- 🔍 **信号B推理引擎**（`should_know.py`）— 从Memory/wiki/项目数据自动提取"你应该知道的"主题清单
- 🔴🟡🟢 **三色标签系统** — 红标（盲区/无相关内容）、黄标（有但过期）、绿标（已有且最新）
- 🌐 **API端点** `/api/analyze/should-know-gaps` — 实时返回认知差距分析结果
- 🖥️ **前端可视化** — 记忆洞察页面新增"🔍 认知差距"tab，展示汇总卡片+主题列表+每日提醒（≤3条）
- 📊 **晚间cron集成** — 板块5每天19:00自动推送认知差距提醒

### Changed
- 记忆洞察面板从4个子视图扩展为5个（生命周期+管理+DNA+衰减+认知差距）
- `engines_mixin.py` — 新增 `_handle_should_know_gaps()` 方法
- 晚间系统整理 skill 新增板块5（认知差距提醒）

### Notes
- 灵感来源：坦哥2021年旧文《探针网络与信息关系网》→ 发现"我知道什么"是不够的，应该先界定"你应该知道什么"
- 首次运行结果：17个认知主题，16个绿标，1个红标（职业生涯复盘）
- 红标入库后自动消失，无需手动操作
