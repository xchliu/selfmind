<p align="center">
  ...
  <p align="center"><strong>Agent DNA Sequencer — Track Evolution, Drive Progress</strong></p>
  <p align="center">AI Agent memory process-state management — Visible · Trackable · Feedback-driven</p>
  <p align="center">Record memory creation, version changes, updates, and decay — let each Agent form its own DNA</p>
  <p align="center">
...
- [x] 多Agent切换（左上角下拉菜单一键切换）
- [x] Gateway自动发现（输入地址探测Agent信息）
- [ ] 修复边关系逻辑（当前0条边）
- [ ] 浅色主题重写
...
**Agent DNA Sequencer — Track Evolution, Drive Progress**
...
- 🔀 **Multi-Agent Switching** — One-click switch between agents (Socrates, Xiaoya, Grace), each with its own memory graph
- 📡 **Gateway Discovery** — Enter Gateway URL to auto-detect agent info, verify paths, and add with one click
...
| `/api/agents/config` | GET | Agent list & config (with Gateway status) |
| `/api/agents/{id}/switch` | PUT | Switch active Agent (re-sync + rebuild graph) |
| `/api/agents/discover` | GET/POST | Gateway auto-discovery for Agent info |
