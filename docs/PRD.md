...
│  D3.js (Force Graph) · 5个视图模块           │
└──────────────────┬──────────────────────────┘
                   │ HTTP API (JSON)
┌──────────────────┴──────────────────────────┐
│                  Backend                     │
│  Python stdlib HTTPServer                    │
│  4个 Handler Mixin（stats/mutations/         │
│    engines/v1） + wiki_parser                │
└──────────────────┬──────────────────────────┘
                   │ Unified Data Pipeline
┌──────────────────┴──────────────────────────┐
│              Data Layer                      │
│  SQLite (unified_store) ← 统一数据源         │
│  Providers: file/skills/aggregation          │
│  MEMORY.md / USER.md + Honcho API            │
...
│   ├── http_handler.py # API 路由核心（476行）
│   ├── unified_store.py # 统一 SQLite 存储层
│   ├── unified_sync.py  # 统一采集入口（memory/user/skill → SQLite）
│   ├── metadata_db.py  # 遗留（待合并到 unified_store）
│   ├── memory_store.py # 遗留（待合并到 unified_store）
...
│       └── aggregation.py
├── static/             # 前端静态资源
│   ├── css/            # global.css, graph.css, health.css, wiki.css, sediment.css
│   └── js/             # app.js, graph.js, views.js, wiki.js, sediment.js, init.js
├── assets/
│   └── logo.png
...
```
1. 启动: server.py → 初始化 UnifiedStore → unified_sync 采集数据
   (memory/user/skill → Providers → SQLite)
...
6. 视图: 记忆图谱 / Wiki库 / 记忆健康 / AI分析 / 记忆沉淀 — 5个独立视图模块
7. 交互: 搜索、筛选、拖拽、查看详情、Wiki编辑、沉淀操作
```
...
| 视图 | JS模块 | CSS模块 | 说明 |
|------|---------|---------|------|
| 记忆图谱 | graph.js | graph.css | D3.js 力导向图 |
| Wiki库 | wiki.js | wiki.css | Wiki 页面浏览与编辑 |
